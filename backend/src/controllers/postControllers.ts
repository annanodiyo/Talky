...
// used middleware to chaeck the account status
...
//used a query in the fn to check account status
// export const createPost = async (req: Request, res: Response) => {
//   try {
//     let { post_text, image_url, user_id } = req.body;
...
//     //   return res.status(400).json({ error: error.details });
//     // }
//     const createStatus = (await dbhelper.query(`SELECT * FROM users where user_id = '${user_id}'`)).recordset
//     if(createStatus[0].is_Deleted == false){
//        const post_id = v4();
...
//         }
//     }else{
//        return res.status(500).json({ message: "activate account to proceed" });
//     }
